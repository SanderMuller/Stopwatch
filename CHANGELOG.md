...
- Each toolbar pill now shows a tooltip that names the metric it reports: total request time against the slow threshold, memory change from start to finish, query count with total query time, and outgoing HTTP request count with total time.
- Toolbar tooltips open on hover and on keyboard focus, and stay inside the viewport in all four toolbar positions.

**Full Changelog**: https://github.com/SanderMuller/Stopwatch/compare/v0.13.1...v0.14.0
