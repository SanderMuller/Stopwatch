...
Config: `STOPWATCH_TRACK_QUERIES=true`. Requires `illuminate/database`. Up to 50 statements per checkpoint are kept with bindings and per-query duration, shown when you expand a row in the [HTML report](06-html-report.md), which is what identifies the slow statement itself.
